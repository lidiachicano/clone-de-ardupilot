...
int lua_serial_find_simulated_device(lua_State *L) {
    ...
    for (int i=0; i<AP_SCRIPTING_SERIALDEVICE_NUM_PORTS; i++) {
        AP_Scripting_SerialDevice::Port *curr_port = &scripting->_serialdevice.ports[i];
        if (curr_port->state.protocol == protocol) {
            if (instance-- == 0) {
                device_stream = curr_port;
                break;
            }
        }
    }
